...
def calcular_stake_sugerida(odd_total):
    if odd_total is None or odd_total <= 1.0:
        return None
...
def status_text():
    ...
    if pb:
        ...
        if pb.get("odd_total") is not None:
            odd_reg = str(pb["odd_total"])
...
def handle_command(text):
    ...
    if t.startswith("/oddreal "):
        if state["pending_bet"] is None:
            send_telegram("ℹ️ Não há pendente para definir odd real.")
            return

        valor_txt = raw.split(" ", 1)[1].strip().replace(",", ".")
        if not is_number(valor_txt):
            send_telegram("❌ Use assim: /oddreal 1.30")
            return

        odd_real = round(float(valor_txt), 2)
        if odd_real <= 1.0:
            send_telegram("❌ Odd inválida.")
            return

        state["pending_bet"]["odd_total"] = odd_real
        stake_sugerida = calcular_stake_sugerida(odd_real)
        state["pending_bet"]["stake_sugerida"] = stake_sugerida
        save_state(state)

        if stake_sugerida is None:
            send_telegram("❌ Não consegui calcular stake.")
            return
        ...
        if not ok_limite:
            send_telegram(
                "🚫 Stake sugerida ultrapassa o limite diário.\n"
                f"Odd real: {odd_real}\n"
                f"Stake necessária: {fmt_money(stake_sugerida)}\n"
                f"Restante do limite: {fmt_money(restante)}"
            )
            return
        ...
        send_telegram(
            ...
            "Agora digite o valor da aposta para registrar."
        )
        return
    ...
    if is_number(raw):
        ...
        if state["pending_bet"].get("odd_total") is None:
            send_telegram(
                "⚠️ Esse pendente está sem odd.\n"
                "Primeiro mande algo como:\n"
                "/oddreal 1.30"
            )
            return
...
def send_manual_alert(cand):
    ...
    msg = (
        ...
        f"Stake base: {fmt_money(get_stake_base())}\n\n"
        "Confira a odd na casa e depois envie:\n"
        "/oddreal 1.30\n\n"
        "Depois disso o bot calcula a stake sugerida.\n\n"
